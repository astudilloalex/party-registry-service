...
- Every new or modified Java class, interface, record, and enum, including tests, must be checked with SonarQube for IDE in Antigravity IDE standalone mode and with JDTLS/LSP diagnostics. Delivery requires zero unresolved findings from the active locally supported Sonar rules in those analyzed files and zero new compiler or IDE warnings, without unjustified suppressions. Record the extension version, active local rule configuration, analyzed revision/files, and actual analysis results separately from Java diagnostics. This local verification requires no SonarQube Server/Cloud, scanner integration, credentials, or remote Quality Gate and does not claim equivalent analysis coverage. Verification evidence must also include contract tests, architecture checks, applicable native integration coverage, `./gradlew test`, and `./gradlew build`.
