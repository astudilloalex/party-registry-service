...
This replaces the DBML's currently requested active-row partial unique indexes. The final DBML must
be amended and independently revalidated before Flyway migration design. No migration may infer
this change from this document alone.
...
The final DBML must add this approved uniqueness rule and replace its current outbox audit note with
the approved `User-Id` semantics before Flyway migration design.
...
provided ISO alpha-2 codes to an outbound port and receives one of these outcomes:
