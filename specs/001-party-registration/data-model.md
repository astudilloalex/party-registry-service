...
provided ISO alpha-2 codes and the trusted audit subject to an outbound port and receives one of
these outcomes:
...
is unknown. The adapter maps trusted `User-Id` to provider `user-id`, generates and verifies one
`process-id` (autogenerated if header not present in this service `Process-Id`) per validation invocation, the `company-id` is `Tenant-ID` for the moment sent `Tenant-Id` header and omit company-id in the request to the geographic service.
