...
The API obtains tenant and audit context from trusted headers, maps a closed request variant to a
framework-free Party aggregate, validates distinct country references through a reactive outbound
port, and persists the aggregate plus an optional `party.created.v1` outbox row atomically.
...
Flyway; PostgreSQL JDBC driver isolated to Flyway; Flyway PostgreSQL support
...
│   ├── context/                     # Tenant-Id and User-Id extraction/validation
...
`PartyResource` validates singular context headers and a closed request variant, sets reactive MDC,
maps to `CreatePartyCommand`, invokes `CreatePartyUseCase`, and maps the committed result to `201`.
One exception mapper converts every expected failure category to the OpenAPI RFC 9457 contract.
